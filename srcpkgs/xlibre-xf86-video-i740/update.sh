...
LATEST_VERSION=$(gh release list --repo X11Libre/xf86-video-i740 --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R X11Libre/xf86-video-i740 --archive=tar.gz --output "${VERSION}.tar.gz"
