...
LATEST_VERSION=$(gh release list --repo X11Libre/xf86-video-intel --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R X11Libre/xf86-video-intel  --archive=tar.gz --output "${VERSION}.tar.gz"
